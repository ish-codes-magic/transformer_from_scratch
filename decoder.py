...
class DecoderBlock(nn.Module):
    def __init__(self, self_attention_block: MultiHeadAttention, cross_attention_block: MultiHeadAttention, feed_forward_block: FeedForwardBlock, dropout: float ) -> None:
        ...
        self.residual_connection = nn.ModuleList([ResidualConnections(dropout) for _ in range(3)])    #3 residual connections for decoder block(makes it easier to pass a function like the self attention block function in the next step)
